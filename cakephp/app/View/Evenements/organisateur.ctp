...
if(!empty($participants)):				
	echo $this->Form->create('Organisateur', array('type' => 'post'));
		...
		// echo $this->Form->label('participant_id', "Nom de l'organisateur");
		echo $this->Form->input('participant_id', array(
					'options' => $participants,
					'label' => 'Organisateur',
					));
		
		// echo $this->Form->label('nom_role', 'Type de droit');
		...
		foreach($organisateurs as $organisateur):
			...
			<li>
				...
				echo ucfirst($organisateur['Participant']['prenom_participant']).' '.ucwords($organisateur['Participant']['nom_participant'])
					.'('.$this->Form->input('nom_role_'.$organisateur['Participant']['participant_id'], array(
									'options' => $type_organisateur,
...
if(empty($categories)):
	...
	<ul>
		...
		foreach($categories as $categorie):
			...
			<li>
				<?php 
				echo $this->Form->checkbox('categorie_ids_'.$i, array(
						'value' => $categorie['categorie_id'],
						'hiddenField' => false,
						));
				echo $this->Form->label('categorie_ids_'.$i, $categorie['nom_categorie']);
				?>
			</li>
			<?php
			$i++;
